...
import { t as require_codemirror } from "./codemirror-BOjlsQc_.js";
//#region node_modules/react-codemirror2/index.js
var require_react_codemirror2 = /* @__PURE__ */ __commonJSMin(((exports) => {
	function _extends() {
	...
	var cm;
	if (!SERVER_RENDERED) cm = require_codemirror();
	var Helper = function() {
		function Helper() {}
	...
	var Shared = function() {
		...
		Shared.prototype.delegateSelection = function(ranges, focus) {
			this.editor.getDoc().setSelections(ranges);
			if (focus) this.editor.focus();
		};
		Shared.prototype.apply = function(props) {
			if (props && props.selection && props.selection.ranges) this.delegateSelection(props.selection.ranges, props.selection.focus || false);
			if (props && props.cursor) this.delegateCursor(props.cursor, props.autoScroll || false, this.editor.getOption("autofocus") || false);
			if (props && props.scroll) this.delegateScroll(props.scroll);
		};
		Shared.prototype.applyNext = function(props, next, preserved) {
			if (props && props.selection && props.selection.ranges) {
				if (next && next.selection && next.selection.ranges && !Helper.equals(props.selection.ranges, next.selection.ranges)) this.delegateSelection(next.selection.ranges, next.selection.focus || false);
			}
			if (props && props.cursor) {
				if (next && next.cursor && !Helper.equals(props.cursor, next.cursor)) this.delegateCursor(preserved.cursor || next.cursor, next.autoScroll || false, next.autoCursor || false);
			}
			if (props && props.scroll) {
				if (next && next.scroll && !Helper.equals(props.scroll, next.scroll)) this.delegateScroll(next.scroll);
			}
		};
		Shared.prototype.applyUserDefined = function(props, preserved) {
			if (preserved && preserved.cursor) this.delegateCursor(preserved.cursor, props.autoScroll || false, this.editor.getOption("autofocus") || false);
		};
	...
	exports.Controlled = function(_super) {
		...
		function Controlled(props) {
			...
			_this.initCb = function() {
				if (_this.props.editorDidConfigure) _this.props.editorDidConfigure(_this.editor);
			};
			_this.mounted = false;
			return _this;
		}
		Controlled.prototype.hydrate = function(props) {
			...
			if (Object.keys(userDefinedOptions).some(function(key) {
			...
			if (!this.hydrated) this.deferred ? this.resolveChange(props.value) : this.initChange(props.value || "");
			this.hydrated = true;
		};
		...
		Controlled.prototype.resolveChange = function(value) {
			this.emulating = true;
			var doc = this.editor.getDoc();
			if (this.deferred.origin === "undo") doc.undo();
			else if (this.deferred.origin === "redo") doc.redo();
			else doc.replaceRange(this.deferred.text, this.deferred.from, this.deferred.to, this.deferred.origin);
		...
		Controlled.prototype.mirrorChange = function(deferred) {
			...
			if (deferred.origin === "undo") {
				...
				doc.setHistory(this.mirror.getHistory());
				this.mirror.redo();
			} else this.mirror.replaceRange(deferred.text, deferred.from, deferred.to, deferred.origin);
		...
		Controlled.prototype.componentDidMount = function() {
			...
			if (this.props.defineMode) {
				if (this.props.defineMode.name && this.props.defineMode.fn) cm.defineMode(this.props.defineMode.name, this.props.defineMode.fn);
			}
			...
			this.editor.on("change", function(cm, data) {
				...
				if (_this.props.onChange) _this.props.onChange(_this.editor, data, _this.editor.getValue());
			});
			...
			if (this.editor.getOption("autofocus")) this.editor.focus();
			if (this.props.editorDidMount) this.props.editorDidMount(this.editor, this.editor.getValue(), this.initCb);
		};
		Controlled.prototype.componentDidUpdate = function(prevProps) {
			if (SERVER_RENDERED) return;
			var preserved = { cursor: null };
			if (this.props.value !== prevProps.value) this.hydrated = false;
			if (!this.props.autoCursor && this.props.autoCursor !== void 0) preserved.cursor = this.editor.getDoc().getCursor();
			this.hydrate(this.props);
			if (!this.appliedNext) {
		...
		Controlled.prototype.componentWillUnmount = function() {
			if (SERVER_RENDERED) return;
			if (this.props.editorWillUnmount) this.props.editorWillUnmount(cm);
			this.hydrated = false;
		};
	...
	exports.UnControlled = function(_super) {
		...
		UnControlled.prototype.hydrate = function(props) {
			...
			if (Object.keys(userDefinedOptions).some(function(key) {
				...
				if (_options.hasOwnProperty(key)) {
					if (_this.editor.getOption(key) !== userDefinedOptions[key]) _this.editor.setOption(key, userDefinedOptions[key]);
				}
			});
			if (!this.hydrated) {
				var doc = this.editor.getDoc();
		...
		UnControlled.prototype.componentDidMount = function() {
			...
			this.shared = new Shared(this.editor, this.props);
			this.editor.on("beforeChange", function(cm, data) {
				if (_this.props.onBeforeChange) _this.props.onBeforeChange(_this.editor, data, _this.editor.getValue(), _this.onBeforeChangeCb);
			});
			this.editor.on("change", function(cm, data) {
				if (!_this.mounted || !_this.props.onChange) return;
				if (_this.props.onBeforeChange) {
					if (_this.continueChange) _this.props.onChange(_this.editor, data, _this.editor.getValue());
				} else _this.props.onChange(_this.editor, data, _this.editor.getValue());
			});
			this.hydrate(this.props);
			this.shared.apply(this.props);
			this.applied = true;
		...
		UnControlled.prototype.componentDidUpdate = function(prevProps) {
			if (this.detached && this.props.detach === false) {
				this.detached = false;
				if (prevProps.editorDidAttach) prevProps.editorDidAttach(this.editor);
			}
			if (!this.detached && this.props.detach === true) {
				this.detached = true;
				if (prevProps.editorDidDetach) prevProps.editorDidDetach(this.editor);
			}
			...
			if (!prevProps.autoCursor && prevProps.autoCursor !== void 0) preserved.cursor = this.editor.getDoc().getCursor();
			this.hydrate(this.props);
			if (!this.applied) {
